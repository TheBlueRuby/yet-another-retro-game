...
pub fn draw_tiles(
    ...
    for y in 0..map.height as i32 {
        for x in 0..map.width as i32 {
            let does_tile_exist = tile_layer.get_tile(x, y);

            match does_tile_exist {
                None => {}
                Some(tile) => {
                    let tile_id = tile.id();
...
pub fn check_tile_at(map: &Map, x: i32, y: i32) -> Option<u32> {
    let tile_layer = map.get_layer(0).unwrap().as_tile_layer().unwrap();
    tile_layer.get_tile(x, y).map(|tile| tile.id())
}
